...
if user_input == 3 :
    ...
    art = soup1.findAll(class_='chart-row__artist')
    for i in range(len(li)):
        fw.write(li[i].text+' '+art[i].text.strip()+'\n')
    fw.close()
    ...
    for x in songs:
        ...
        with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            ydl.download(['https://www.youtube.com'+title[0].find('a')['href']])
            print ('Downloaded.')
            fa.write(x+'\n')
...
#Search and download songs
    ...
    with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            ydl.download(['https://www.youtube.com'+link[user_input - 1]])
            print ('Downloaded.')
    print ('Download Complete')   
